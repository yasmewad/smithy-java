...
import software.amazon.smithy.java.runtime.serde.TimestampFormatter;
...
final class JsonAny implements Any {
    ...
    public List<Any> asList() {
        ...
        SdkSchema member = schema.member("member", SCHEMA);
    ...
    public Map<Any, Any> asMap() {
        if (any.valueType() != ValueType.OBJECT) {
            ...
            SdkSchema keyMember = schema.member("key", SCHEMA);
            SdkSchema valueMember = schema.member("value", SCHEMA);
    ...
    public Any getStructMember(String memberName) {
        SdkSchema member = schema.member(memberName, SCHEMA);
        if (any.valueType() == ValueType.OBJECT) {
