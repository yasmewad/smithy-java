...
package software.amazon.smithy.java.runtime.auth;
...
public final class SigV4 {
    ...
    public Map<String, List<String>> createSignedHeaders(
        ...
        return createSignedHeaders(request, accessKeyId, secretKey, payload, isStreaming, Instant.now());
    }
    ...
    public Map<String, List<String>> createSignedHeaders(
        ...
        var hostHeader = request.uri().getHost();
        if (request.uri().getPort() > 0) {
            hostHeader += ":" + request.uri().getPort();
        }
        ...
        var signedHeadersAuthorizationHeader = "SignedHeaders=" + getSignedHeaders(request);
    ...
    private String getSignedHeaders(SmithyHttpRequest request) {
        // Sort header names/keys
        Set<String> sortedHeaders = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
        sortedHeaders.addAll(request.headers().map().keySet());
    ...
    private String getCanonicalizedHeaderString(SmithyHttpRequest request) {
        Set<String> sortedHeaders = new TreeSet<>(String.CASE_INSENSITIVE_ORDER);
        sortedHeaders.addAll(request.headers().map().keySet());
    ...
    private String getCanonicalizedQueryString(SmithyHttpRequest request) {
